...
def preprocess_decision(input_data, llm):
    ...
    - Не включай колонку "Цена" в словари fill_missing и outlier_actions (для этих словарей работаем только с признаками)
    ...
    Если у целевой колонки "Цена" выраженная правосторонняя асимметрия или тяжелый правый хвост,
    можно в notes кратко предложить мягкие варианты нормализации распределения цели (например, log1p-преобразование или аккуратная работа с верхним хвостом),
    без жестких предписаний и без добавления "Цена" в outlier_actions/fill_missing.
