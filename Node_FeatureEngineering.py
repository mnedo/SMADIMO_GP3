        ...
        response = llm.invoke([
            SystemMessage(
                content=(
                    ...
                    'Применяй осторожное кодирование категорий: для низкой кардинальности чаще уместен one-hot, для более высокой — более компактные подходы (например, частотное/порядковое кодирование), чтобы не раздувать шумные sparse-признаки. '
            ...
            HumanMessage(
                content=(
                    ...
                    'Обработай категориальные признаки, например, материал одежды, материал джинсов, материал футболок и другие материалы можно объединить в одну колонку, поскольку это одно и тоже и применить энкодинг. Для цвета можно тожно применить, например, onehot encoding. Категорию и продавца тоже неоходимо привести к числовому признаку через кодирование. Используй доступный EDA-контекст и самостоятельно выбери разумные преобразования для создания новых признаков в df. Можешь в том числе получить новый признак при помощи кластеризации данных с помощью sklearn.cluster.KMeans или другого алгоритма кластеризации. '
